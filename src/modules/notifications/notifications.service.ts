...
export class NotificationsService {
  ...
  async sendNotification(notification: Notification): Promise<boolean> {
    try {
      ...
      await this.updateNotificationStatus(notification.notificationId, NotificationStatus.FAILED);
      return false;
    }
  }
  ...
  private async sendEmailNotification(notification: Notification): Promise<boolean> {
    try {
      ...
      this.logger.log(`Email notification sent successfully to user ${notification.userId}`);
      return true;
    } catch (error) {
  ...
  private async sendPushNotification(notification: Notification): Promise<boolean> {
    try {
      ...
      this.logger.log(`Push notification would be sent: ${notification.title} - ${notification.message}`);
  ...
  private async updateNotificationStatus(notificationId: number, status: NotificationStatus): Promise<void> {
